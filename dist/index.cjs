...
var validate$1 = require('@adobe/reactor-validator');
...
const extensionDescriptor = JSON.parse(fs.readFileSync(extensionJsonPath, 'utf8'));
...
const require$2 = node_module.createRequire ? node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href))) : undefined;
...
class Y18N {
    ...
    _readLocaleFile() {
        ...
        try {
            ...
            if (shim$1.fs.readFileSync) {
                localeLookup = JSON.parse(shim$1.fs.readFileSync(languageFile, 'utf-8'));
            }
        }
        catch (err) {
...
const __dirname$1 = url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('index.cjs', document.baseURI).href)));
...
var esmPlatformShim = {
  ...
  getCallerFile: () => {
    ...
    return callerFile.match(/^file:\/\//) ? url.fileURLToPath(callerFile) : callerFile;
  },
  stringWidth,
  y18n: y18n({
...
var requiresRegex;
var hasRequiredRequiresRegex;

function requireRequiresRegex () {
	if (hasRequiredRequiresRegex) return requiresRegex;
	hasRequiredRequiresRegex = 1;

	requiresRegex = function() {
	  return /(?:(?:\\['"`][\s\S])*?(['"`](?=[\s\S]*?require\s*\(['"`][^`"']+?[`'"]\)))(?:\\\1|[\s\S])*?\1|\s*(?:(?:var|const|let)?\s*([_.\w/$]+?)\s*=\s*)?require\s*\(([`'"])((?:@([^/]+?)\/([^/]*?)|[-.@\w/$]+?))\3(?:, ([`'"])([^\7]+?)\7)?\);?)/g;
	};
	return requiresRegex;
}

var requiresRegexExports = requireRequiresRegex();
var re = /*@__PURE__*/getDefaultExportFromCjs(requiresRegexExports);

function matchRequires(str, options) {
  options = options || {};
...
var getAvailableTypes = function(descriptor) {
  ...
  return R__namespace.intersection(allTypes, R__namespace.keys(descriptor));
};
...
var getLibPaths = function(descriptor) {
  ...
  var getPaths = R__namespace.compose(
    R__namespace.concat(ensureArray(descriptor.iconPath)),
    R__namespace.concat(ensureArray(descriptor.hostedLibFiles)),
    R__namespace.reduce(recursivelyAccumulateRequiredPaths, []),
    R__namespace.concat(ensureArray(descriptor.main)),
    R__namespace.pluck('libPath'),
    R__namespace.flatten,
    R__namespace.props(getAvailableTypes(descriptor))
  );
...
const argv = Yargs(helpers.hideBin(process$1.argv))
...
try {
  var error = validate$1(extensionDescriptor);
